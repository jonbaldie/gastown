...
- For Codex, Gas Town writes `AGENTS.md` as the canonical instruction file.
  `project_doc_fallback_filenames = ["CLAUDE.md"]` is optional; `CLAUDE.md` is a
...
**Built-in agent presets**: `claude`, `gemini`, `codex`, `kiro`, `cursor`, `auggie`, `amp`, `opencode`, `copilot`, `pi`, `omp`, `agy`. Mix them in one town with `gt config mix default=codex mayor=pi`.
