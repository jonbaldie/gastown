...
- **Integration tests use a native Dolt SQL server and run in one process.**
  ...
  test uses both). Assertions are unchanged. CI runs the `gt now` five-second
  contract before the race suite so a hot runner cannot miss the budget.
...
- **`go install ...@latest` is this module.** Docs, README, uninstall, and
  release notes install `gt` from
