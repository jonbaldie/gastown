...
func closeTerminalMR(b *beads.Beads, mrID string, opts terminalMRCloseOptions) (*terminalMRCloseResult, error) {
	...
	issue, fields, err := loadTerminalMR(b, mrID, opts.MissingOK)
	...
	switch status := beads.IssueStatus(strings.TrimSpace(issue.Status)); {
	case status == beads.StatusOpen:
		if err := closeOpenTerminalMR(b, mrID, issue, fields, opts, result.AgentBead); err != nil {
			return result, err
		}
		result.Closed = true
	...
	clearTerminalMRAgent(b, mrID, result)
	return result, nil
}

func clearTerminalMRAgent(b *beads.Beads, mrID string, result *terminalMRCloseResult) {
	if result.AgentBead == "" {
		return
	}
...
func loadTerminalMR(b *beads.Beads, mrID string, missingOK bool) (*beads.Issue, *beads.MRFields, error) {
	issue, err := b.Show(mrID)
	if err != nil {
		if errors.Is(err, beads.ErrNotFound) && missingOK {
			return nil, nil, nil
		}
		return nil, nil, fmt.Errorf("fetch MR for close: %w", err)
	}
	if issue == nil {
		return nil, nil, nil
	}
...
func closeOpenTerminalMR(b *beads.Beads, mrID string, issue *beads.Issue, fields *beads.MRFields, opts terminalMRCloseOptions, agentBead string) error {
	if opts.MergeCommit != "" {
	...
	if agentBead != "" && strings.TrimSpace(fields.AgentBead) == "" {
		fields.AgentBead = agentBead
	}
