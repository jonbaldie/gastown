...
const (
	...
	primeToSpecDirective             = "Bead epics: use /to-spec's SKILL.md rigorously."
	primeToTicketsDirective          = "Bead children: use /to-tickets's SKILL.md rigorously."
	primeBeadsRuleOfThumb            = "Rule of thumb: synthesize one parent spec; children are one-window vertical slices that declare blockers."
...
func outputSkillDirectives(w io.Writer) {
	...
	fmt.Fprintln(w, primeBeadsRuleOfThumb)
	fmt.Fprintln(w, primeConflictResolutionDirective)
}
...
func outputMayorContext(ctx RoleContext) {
	...
	fmt.Println()
	fmt.Println("## Rig work beads")
	fmt.Println("Create work IN the target rig. Do not file town `hq-*` work beads and sling them.")
	fmt.Println()
	fmt.Println("```bash")
	...
	fmt.Println("```")
	fmt.Println()
	fmt.Println("Use `--merge=local` (or `--push-url`) for third-party remotes.")
	fmt.Println("Do not sling infrastructure beads (`hq-cv-*`, `hq-mayor`).")
	fmt.Println()
