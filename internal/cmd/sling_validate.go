...
func ValidateTarget(target string) error {
	...
	if err := validateTargetSegments(target, parts); err != nil {
		return err
	}
	...
	return validateRigTarget(target, parts)
}

func validateTargetSegments(target string, parts []string) error {
...
func validateRigTarget(target string, parts []string) error {
	...
	if !knownRoles[role] {
		return validateUnknownRoleTarget(target, parts)
	}
	return validateKnownRoleTarget(target, parts, role)
}

func validateUnknownRoleTarget(target string, parts []string) error {
	if len(parts) <= 2 {
		// Two-segment with unknown role → polecat/crew shorthand, let resolveTarget handle.
		return nil
	}
...
func validateKnownRoleTarget(target string, parts []string, role string) error {
