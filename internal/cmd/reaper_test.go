...
func TestReaperDatabaseNamesTrimsConfiguredList(t *testing.T) {
	got := reaperDatabaseNames(reaperOptions{db: " hq, gastown ,, beads "})
...
func TestWaitBeforeReaperDatabase(t *testing.T) {
	if err := waitBeforeReaperDatabase(0, "0s"); err != nil {
		t.Fatalf("first database wait returned error: %v", err)
	}
	if err := waitBeforeReaperDatabase(1, "0s"); err != nil {
		t.Fatalf("zero-delay wait returned error: %v", err)
	}

	if err := waitBeforeReaperDatabase(1, "not-a-duration"); err == nil {
